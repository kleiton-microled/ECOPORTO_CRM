...
namespace WsSimuladorCalculoTabelas.DAO
{
    public class ServicoDAO
    {
        ...
        public IEnumerable<ServicoFixoVariavel> ObterServicosLCLPorTabela(int tabelaId)
        {
        ...
        public IEnumerable<ServicoFixoVariavel> ObterServicosFCLPorTabela(int tabelaId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
        ...
        public bool PeriodoEscalonado(int oportunidadeId, int periodo)
        {
            using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
            {
                ...
                return con.Query<bool>($@"SELECT Id FROM TB_CRM_OPORTUNIDADE_LAYOUT WHERE PERIODO = :Periodo AND OPORTUNIDADEID = :OportunidadeId AND TipoRegistro = :TipoRegistro", parametros).Any();
            }
        }
        ...
        public IEnumerable<ServicoFixoVariavel> ObterAdicionais(int tabelaId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    return con.Query<ServicoFixoVariavel>($@"
                        ...
                        WHERE 
                            B.LISTA = :TabelaId 
                        AND 
                            B.VALOR_ACRESCIMO > 0 AND A.AUTONUM <> 295 AND A.FLAG_TAXA_LIBERACAO=0 
                        ...
                        WHERE 
                            B.LISTA = :TabelaId 
                        AND 
                            B.VALOR_ACRESCIMO > 0 AND A.AUTONUM <> 295 AND A.FLAG_TAXA_LIBERACAO=0 ", parametros);
                }
            }
            else
            {
        ...
        public IEnumerable<ServicoFixoVariavel> ObterAdicionaisAnvisa(int tabelaId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
        ...
        public decimal ObterAdicionalArmazenagem(int tabelaId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    return con.Query<decimal>($@"SELECT NVL(MAX(PRECO_UNITARIO),0) PrecoUnitario FROM {_schema}.TB_LISTA_P_S_PERIODO WHERE LISTA = :TabelaId AND SERVICO = 295", parametros).FirstOrDefault();
                }
            }
            else
            {
