...
namespace WsSimuladorCalculoTabelas.DAO
{
    public class TabelasDAO
    {
        ...
        public ServicoFixoVariavel ObterServicoPeriodoPorLinha(int linha, int oportunidadeId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
            ...
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    else
                    {
                        return con.Query<int>($@"
                        SELECT AUTONUM FROM {_schema}.TB_LISTA_P_S_PERIODO WHERE SERVICO = 52 AND TIPO_CARGA = :TipoCarga 
                            AND BASE_CALCULO = :BaseCalculo AND LISTA = :Lista AND LINHA <= :Linha", parametros);
                    }
                }
            }
            else
        ...
        public void GravarLotes(int tabelaId, string[] lotes)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
        ...
        public void ExportarLotes(int tabelaCobranca, int oportunidadeId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    if (!string.IsNullOrEmpty(lotesProposta))
                    {
                        ...
                        GravarLotes(tabelaCobranca, lotes);
                    }
                }
            }
            else
            ...
            using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
            {
                ...
                return con.Query<bool>($@"
                    ...
                    {_schema}.TB_LISTA_P_S_PERIODO WHERE LINHA = :Linha AND SERVICO = :ServicoId 
                        ...
                        AND BASE_CALCULO = :BaseCalculo AND N_PERIODO = :Periodo AND OPORTUNIDADEID = :OportunidadeId", parametros).Any();
            }
        }
        ...
        public void GravarServicoFixosLiberacao(int tabelaId, int oportunidadeId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    var TemCntr = con.Query(@"SELECT AUTONUM FROM SGIPA.TB_LISTA_P_S_PERIODO 
                             WHERE Lista = :TabelaId AND Servico = 52 and TIPO_CARGA <>'CRGST'", parametrosT).Any();

                    var TemCs = con.Query(@"SELECT AUTONUM FROM SGIPA.TB_LISTA_P_S_PERIODO 
                             WHERE Lista = :TabelaId AND Servico = 52 and TIPO_CARGA ='CRGST'", parametrosT).Any();
...

                    {
                        ...
                        foreach (var servicoSincronismo in servicosSincronismocntr)
                        {
                            ...
                            var Auto = con.Query<long>(@"SELECT NVL(MAX(AUTONUM),0) FROM SGIPA.TB_LISTA_PRECO_SERVICOS_FIXOS WHERE Lista = :TabelaId 
        ...
        public void ExcluirServicosEntreMargem(int tabelaId)
        {
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    con.Execute($@"DELETE FROM SGIPA.TB_LISTA_PRECO_SERVICOS_FIXOS WHERE VARIANTE_LOCAL = 'ENTR' AND LISTA = :TabelaId AND SERVICO in( 332,80)", parametros);
                }
            }
        }
            ...
            using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
            {
                ...
                using (var transaction = con.BeginTransaction())
                {
                    ...
                    if (oportunidade.Acordo)
                    {
                        ...
                        parametros.Add(name: "Liberada", value: 1, direction: ParameterDirection.Input);
                    }
                    else
                    {
                    ...
                    if (oportunidade.TabelaId > 0)
                    {
                        ...
                        AtualizarObservacaoTabela(oportunidade.TabelaId, oportunidade.RevisaoId);
        ...
        private static void AtualizarObservacaoTabela(int novaTabelaId, int? oportunidadeRevisaoId)
        {
            using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
            {
            ...
            if (Configuracoes.BancoEmUso() == "ORACLE")
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    while (saida)
                    {
                        ...
                        var duplicidadeper = con.Query<TabelaDuplicada>(@"
                            SELECT 
                                COUNT(1) Total, 
                                MAX(N_PERIODO) Max, 
                                MIN(N_PERIODO) Min,
                            ...
                            GROUP BY   
                                ...
                                VALOR_ACRESC_PESO,
                                PESO_LIMITE 
                            HAVING COUNT(1) > 1 ORDER BY MAX(AUTONUM) DESC", parametros);

                        if (!duplicidadeper.Any())
                            saida = false;

                        foreach (var item in duplicidadeper)
                        {
                            if (item.Max >= item.Min)
                            {
                                ...
                                foreach (var proximo in temproximo)
                                {
                                    ...
                                    parametros.Add(name: "N_Periodo", value: proximo.N_Periodo, direction: ParameterDirection.Input);
            ...
            foreach (var margem in variantes)
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    foreach (var faixa in faixasCif)
                    {
                        ...
                        parametros.Add(name: "OportunidadeId", value: oportunidadeId, direction: ParameterDirection.Input);
                        parametros.Add(name: "TipoRegistroCif", value: TipoRegistro.ARMAZENAGEM_CIF, direction: ParameterDirection.Input);
                        ...
                        if (margem != "SVAR")
                        {
                            ...
                            else
                            {
                             ...
                             valoresCif = con.Query<LayoutDTO>($@"
                                SELECT ValorCif, Valor, Valor20, Valor40, TipoCarga FROM CRM.TB_CRM_OPORTUNIDADE_LAYOUT 
                                    WHERE OportunidadeId = :OportunidadeId AND Margem = :Margem AND TipoRegistro = :TipoRegistroCif ORDER BY Linha", parametros).ToArray();
                        }
                        else
                        {
                            valoresCif = con.Query<LayoutDTO>($@"
                                SELECT ValorCif, Valor, Valor20, Valor40, TipoCarga FROM CRM.TB_CRM_OPORTUNIDADE_LAYOUT 
                                    WHERE OportunidadeId = :OportunidadeId AND TipoCarga = 2 AND TipoRegistro = :TipoRegistroCif ORDER BY Linha", parametros).ToArray();
                        }                        
                        ...
                        if (count < valoresCif.Count() - 1)
                        {
                            if (count == 0)
                            {
                                parametros.Add(name: "ValorInicial", value: valoresCif[count].ValorCif - 0.01M, direction: ParameterDirection.Input);
                                parametros.Add(name: "ValorFinal", value: valoresCif[count + 1].ValorCif - 0.01M, direction: ParameterDirection.Input);
                            }
                            else
                            {
                                parametros.Add(name: "ValorInicial", value: valoresCif[count].ValorCif, direction: ParameterDirection.Input);
                                parametros.Add(name: "ValorFinal", value: valoresCif[count + 1].ValorCif - 0.01M, direction: ParameterDirection.Input);
                            }

                            if (faixa.TipoCarga == "SVAR20")
                            {
                                parametros.Add(name: "Valor", value: valoresCif[count].Valor20, direction: ParameterDirection.Input);
                            }
                            else if (faixa.TipoCarga == "SVAR40")
                            {
                                parametros.Add(name: "Valor", value: valoresCif[count].Valor40, direction: ParameterDirection.Input);
                            }
                            else
                            {
                                if (valoresCif[count].TipoCarga == TipoCarga.CONTEINER || valoresCif[count].TipoCarga == TipoCarga.MUDANCA_REGIME)
                                {
                                    parametros.Add(name: "Valor", value: valoresCif[count].Valor20, direction: ParameterDirection.Input);
                                }
                                else
                                {
                                    parametros.Add(name: "Valor", value: valoresCif[count].Valor, direction: ParameterDirection.Input);
                                }
                            }

                            parametros.Add(name: "Id", value: faixa.Id, direction: ParameterDirection.Input);

                            con.Query($"UPDATE {_schema}.TB_LISTA_P_S_FAIXASCIF_PER SET Percentual = :Valor, ValorInicial = :ValorInicial, ValorFinal = :ValorFinal WHERE AUTONUM = :Id", parametros);
                        }

                        if (count == valoresCif.Count() - 1)
                        {
                            parametros.Add(name: "ValorInicial", value: valoresCif[valoresCif.Count() - 1].ValorCif, direction: ParameterDirection.Input);
                            parametros.Add(name: "ValorFinal", value: 999999, direction: ParameterDirection.Input);
                            ...
                            valorCif = con.Query<LayoutDTO>($@"
                                ...
                                (
                                    SELECT ROWNUM As Linha, ValorCif, ValorMinimo, ValorMinimo20, ValorMinimo40, TipoCarga FROM
                                        (
                                            SELECT 
                                                ValorCif, ValorMinimo, ValorMinimo20, ValorMinimo40, TipoCarga FROM CRM.TB_CRM_OPORTUNIDADE_LAYOUT 
                                            WHERE 
                                                OportunidadeId = :OportunidadeId AND Margem = :Margem AND TipoRegistro = :TipoRegistroCif ORDER BY ValorCif
                                        ) 
                                    ...
                                    SELECT ROWNUM As Linha, ValorCif, ValorMinimo, ValorMinimo20, ValorMinimo40, TipoCarga FROM
                                        (
                                            SELECT 
                                                ValorCif, ValorMinimo, ValorMinimo20, ValorMinimo40, TipoCarga FROM CRM.TB_CRM_OPORTUNIDADE_LAYOUT 
                                            WHERE 
                                                OportunidadeId = :OportunidadeId AND TipoCarga = 2 AND TipoRegistro = :TipoRegistroCif ORDER BY ValorCif
                                        ) 
                                ) WHERE Linha = :Linha", parametros).FirstOrDefault();
                        }                        
