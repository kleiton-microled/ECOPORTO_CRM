            ...
            foreach (var layout in linhas)
            {
                ...
                switch (layout.TipoRegistro)
                {
                            ...
                            if (armazenagem != null)
                            {
                                if (layout.TipoCarga == TipoCarga.CONTEINER)
                                {
                                    foreach (var tipoCarga in tiposCargas)
                                    {
                                        if (tipoCarga == "SVAR20")
                                        {
                                            _tabelaDAO.AtualizarPrecoMinimo(layout.ValorMinimo20, layout.LimiteBls, armazenagem.ServicoFixoVariavelId, tipoCarga, layout.OportunidadeId, VarianteLocal);
                                        }
                                        else if (tipoCarga == "SVAR40")
                                        {
                                            _tabelaDAO.AtualizarPrecoMinimo(layout.ValorMinimo40, layout.LimiteBls, armazenagem.ServicoFixoVariavelId, tipoCarga, layout.OportunidadeId, VarianteLocal);
                                        }
                                    }
                                }
                                ...
                                if (layout.TipoCarga == TipoCarga.MUDANCA_REGIME)
                                {
                                    ...
                                    {                                        
                                        foreach (var tipoCarga in tiposCargas)
                                        {
                                            var ids = _tabelaDAO.ObterServicosArmazenagemAnteriores(armazenagem.Periodo, "CPIER", armazenagem.BaseCalculo, tabelaCobranca, layout.Linha);
                                    ...
                                    {
                                        _tabelaDAO.AtualizarPrecoMinimo(layout.ValorMinimo20, layout.LimiteBls, armazenagem.ServicoFixoVariavelId, "CPIER", layout.OportunidadeId, VarianteLocal);
                                    }
                                }
                                ...
                                if (layout.TipoCarga == TipoCarga.CARGA_SOLTA)
                                {
                                    var ids = _tabelaDAO.ObterServicosArmazenagemAnteriores(
                                        armazenagem.Periodo, armazenagem.TipoCarga, armazenagem.BaseCalculo, tabelaCobranca, layout.Linha);
                                    ...
                                    {
                                        _tabelaDAO.AtualizarPrecoMinimo(layout.ValorMinimo, layout.LimiteBls, armazenagem.ServicoFixoVariavelId, "CRGST", layout.OportunidadeId, VarianteLocal);
                                    }
                                }
                            }
                        }
                    ...
                    case TipoRegistro.MINIMO_MECANICA_MANUAL:
                        ...
                        if (layout.LinhaReferencia > 0)
                        {
                            /*var servicosMecManual = _tabelaDAO.ObterServicoFixoPorLinha(layout.LinhaReferencia, layout.OportunidadeId, "");

                            foreach (var servicoMecManual in servicosMecManual)
                            {
                                 if (layout.ValorMinimo20 != layout.ValorMinimo40)
                                {*/
                            foreach (var tipoCarga in tiposCargas)
                    ...
                    case TipoRegistro.SERVICO_HUBPORT:
                        ...
                        objServicoHubPort.TipoCarga = "SVAR";
                    ...
                    //case TipoRegistro.ARMAZENAGEM_CIF:
                    //    var objPeriodoArmCif = new ServicoFixoVariavel
                    //    {
                    //        Lista = tabelaCobranca,
                    //        OportunidadeId = layout.OportunidadeId,
                    //        Linha = layout.Linha,
                    ...
                    //    if (layout.AdicionalIMO > 0)
                    //    {
                    //        objPeriodoArmCif.ValorAcrescimo = layout.AdicionalIMO;
                    //    }

                    //    if (layout.ValorANVISA > 0)
                    //    {
                    //        objPeriodoArmCif.ValorAnvisa = layout.ValorANVISA;
                    //    }
                    ...
                    //                if (tipoCarga == "SVAR20")
                    //                {
                    //                    objPeriodoArmCif.TipoCarga = "SVAR20";
                    //                    objPeriodoArmCif.PrecoUnitario = layout.Valor20;
                    //                    objPeriodoArmCif.Linha = layout.Linha;                                         
                    //                }
                    //                if (tipoCarga == "SVAR40")
                    //                {
                    //                    objPeriodoArmCif.TipoCarga = "SVAR40";
                    //                    objPeriodoArmCif.PrecoUnitario = layout.Valor40;
                    //                    objPeriodoArmCif.Linha = layout.Linha;
                    //                }
                    //                foreach (var servico in servicosIPA)
                    //                {
                    //                    objPeriodoArmCif.ServicoId = servico.Id;
                    //                    _tabelaDAO.GravarServicoVariavel(objPeriodoArmCif, 1);
                    //                }
                    //            }
                    //        }
                    //        else
                    ...
                    //            if (layout.Valor > 0)
                    //            {
                    //                objPeriodoArmCif.PrecoUnitario = layout.Valor;
                    //            }
                    //            if (layout.Valor20 > 0)
                    //            {
                    //                objPeriodoArmCif.PrecoUnitario = layout.Valor20;
                    //            }
                    //            if (objPeriodoArmCif.PrecoUnitario > 0  )
                    //            {
                    //                objPeriodoArmCif.TipoCarga = "SVAR";
                    //                objPeriodoArmCif.Linha = layout.Linha;
                    //                foreach (var servico in servicosIPA)
                    //                {
                    //                    objPeriodoArmCif.ServicoId = servico.Id;
                    ...
                    //    {
                    //        objPeriodoArmCif.PrecoUnitario = layout.Valor;
                    //        objPeriodoArmCif.TipoCarga = "CRGST";
                    ...
                    //    {
                    //        objPeriodoArmCif.PrecoUnitario = layout.Valor;
                    //        objPeriodoArmCif.TipoCarga = "CPIER";
                    //        if ((layout.Valor > 0) )
                    //        {
                    //            objPeriodoArmCif.PrecoUnitario = layout.Valor;
                    //            foreach (var servico in servicosIPA)
